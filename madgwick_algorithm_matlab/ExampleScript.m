...
fid = fopen('Gx.txt','wt');%数据保存在你当前的文件夹下，文件名为Data.txt
Temp = Gyroscope(:,1);
fprintf(fid,'%f\n',Temp);
fclose(fid);

fid = fopen('Gy.txt','wt');%数据保存在你当前的文件夹下，文件名为Data.txt
Temp = Gyroscope(:,2);
fprintf(fid,'%f\n',Temp);
fclose(fid);

fid = fopen('Gz.txt','wt');%数据保存在你当前的文件夹下，文件名为Data.txt
Temp = Gyroscope(:,3);
fprintf(fid,'%f\n',Temp);
fclose(fid);

fid = fopen('Ax.txt','wt');%数据保存在你当前的文件夹下，文件名为Data.txt
Temp = Accelerometer(:,1);
fprintf(fid,'%f\n',Temp);
fclose(fid);

fid = fopen('Ay.txt','wt');%数据保存在你当前的文件夹下，文件名为Data.txt
Temp = Accelerometer(:,2);
fprintf(fid,'%f\n',Temp);
fclose(fid);

fid = fopen('Az.txt','wt');%数据保存在你当前的文件夹下，文件名为Data.txt
Temp = Accelerometer(:,3);
fprintf(fid,'%f\n',Temp);
fclose(fid);
